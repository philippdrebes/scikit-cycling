...
class RidePowerProfile(BasePowerProfile):
    ...
    def __init__(self, filename=None, max_duration_profile=None,
                 cyclist_weight=None):
    ...
    def fit(self, filename=None):
        ...
        self : object
            Returns self.
        """
        # Check that the file given or which given is valid
        if filename is None and self.filename is None:
            raise ValueError('Not any filename was provided from which to read'
                             ' the data from.')
        elif filename is not None and self.filename is not None:
            raise warnings.warn('The filename was overwritten using the string'
                                ' passed in the function `fit`.')
            self.filename_ = check_filename_fit(filename)
        else:
            if filename is not None:
                self.filename_ = check_filename_fit(filename)
            else:
                self.filename_ = check_filename_fit(self.filename)
        ...
        if self.max_duration_profile is None:
            ...
            self.max_duration_profile_ = self.max_duration_profile
        # Make the processing with all the available processor
        pp = Parallel(n_jobs=-1)(delayed(self._rpp_parallel)(ride_power,
                                                             idx_t_rpp)
                                 for idx_t_rpp
