...
class FitFile(object):
    ...
    def _parse_file_header(self):
        ...
        (header_size, protocol_ver_enc,
         profile_ver_enc, data_size) = self._read_struct(
        ...
        if extra_header_size > 0:
            # Make sure extra field in header is at least 2 bytes to calculate
            # CRC
    ...
    def _parse_definition_message(self, header):
        ...
        for n in range(num_fields):
            ...
            if (field_size % base_type.size) != 0:
                ...
                raise FitParseError(
                    "Invalid field size %d for type '%s' (expected a"
                    " multiple of %d)"
            ...
            for sub_field in field.subfields:
                ...
                for ref_field in sub_field.ref_fields:
                    ...
                    for field_def, raw_value in zip(def_mesg.field_defs,
                                                    raw_values):
                        # If there's a definition number AND raw value match
                        # on the reference field, then we return this
                        # subfield
    ...
    def _parse_data_message(self, header):
        ...
        for field_def, raw_value in zip(def_mesg.field_defs, raw_values):
            ...
            if field:
                ...
                # TODO: Do we care about a base_type and a resolved field
                # mismatch? My hunch is we don't
